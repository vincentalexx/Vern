        ...
        <div class="mt-5 w-[82%]">
            <div class="py-4 flex flex-wrap justify-between">
                ...
                <div class="flex flex-col">
                    ...
                    <input class="border-2 border-black rounded-md w-[400px] mb-3 h-8 px-2" value="{{ $order->vehicle->brand }} {{ $order->vehicle->model }} {{ $order->vehicle->year }}" readonly>
                </div>
                <div class="flex flex-col">
                    <label class="h-7 font-bold">Payment Method</label>
                @if($order->payment_type == 1)
                        <input class="border-2 border-black rounded-md w-[400px] mb-3 h-8 px-2" value="Cash" readonly>
                    @elseif($order->payment_type == 2)
                        <input class="border-2 border-black rounded-md w-[400px] mb-3 h-8 px-2" value="E-Money" readonly>
                    @endif
                </div>
                <div class="flex flex-col">
                    <label class="h-7 font-bold">Status</label>
                    @if($order->status == 1)
                        <input class="border-2 border-black rounded-md w-[400px] mb-3 h-8 px-2" value="Waiting for Payment" readonly>
                    @elseif($order->status == 2)
                        <input class="border-2 border-black rounded-md w-[400px] mb-3 h-8 px-2" value="Waiting for Confirmation" readonly>
                    @elseif($order->status == 3)
                        <input class="border-2 border-black rounded-md w-[400px] mb-3 h-8 px-2" value="In Progress" readonly>
                    @elseif($order->status == 4)
                        <input class="border-2 border-black rounded-md w-[400px] mb-3 h-8 px-2" value="Completed" readonly>
                    @elseif($order->status == 5)
                        <input class="border-2 border-black rounded-md w-[400px] mb-3 h-8 px-2" value="Cancelled" readonly>
                    @endif
                </div>
            </div>
