...
export default function Search({ results, startDate, endDate }) {
    ...
    const filteredResults = results.filter((result) => {
        ...
        const isNaNValue = isNaN(parsedPriceStart) || isNaN(parsedPriceEnd);
        // console.log(result.price, priceStart, priceEnd)
        const meetsPriceRangeFilter =
            isNaNValue ||
            ((parsedPriceStart === 0 && parsedPriceEnd === 0)) || // Both are 0
            (parsedPriceStart === 0 &&
                parsedPriceEnd !== 0 &&
                result.price <= parsedPriceEnd) || // Only priceStart is 0
            (parsedPriceStart !== 0 &&
                parsedPriceEnd === 0 &&
                result.price >= parsedPriceStart) || // Only priceEnd is 0
            (parsedPriceStart !== 0 &&
                parsedPriceEnd !== 0 &&
                result.price >= parsedPriceStart &&
                result.price <= parsedPriceEnd); // Both are not 0
